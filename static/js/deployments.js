...
function setupEventListeners() {
    ...
    if (nsSelect) {
        nsSelect.addEventListener('change', function() {
            ...
            if (currentNsEl) currentNsEl.textContent = currentNamespace;
            loadDeployments();
        });
    }
    
    // Поиск
    const searchEl = document.getElementById('search-deployments');
    if (searchEl) searchEl.addEventListener('input', debounce(filterDeployments, 300));
    ...
    const liveRefreshEl = document.getElementById('deployments-live-refresh');
    if (liveRefreshEl) {
        liveRefreshEl.addEventListener('change', function() {
            if (autoRefreshInterval) clearInterval(autoRefreshInterval);
            autoRefreshInterval = null;
            if (this.checked) autoRefreshInterval = setInterval(loadDeployments, 10000);
        });
    }
    
    // Slider синхронизация
    const replicasSlider = document.getElementById('replicas-slider');
    if (replicasSlider) {
        replicasSlider.addEventListener('input', function() {
            const replicasInput = document.getElementById('replicas-input');
            const replicasValue = document.getElementById('replicas-value');
            if (replicasInput) replicasInput.value = this.value;
            if (replicasValue) replicasValue.textContent = this.value;
        });
    }
    
    const replicasInput = document.getElementById('replicas-input');
    if (replicasInput) {
        replicasInput.addEventListener('input', function() {
            const value = Math.min(20, Math.max(0, parseInt(this.value) || 0));
            const slider = document.getElementById('replicas-slider');
            const valueEl = document.getElementById('replicas-value');
...
async function loadDeployments() {
    ...
    try {
        let url = currentNamespace === 'all' 
            ? '/api/deployments' 
            : `/api/deployments?namespace=${currentNamespace}`;
        const labelsEl = document.getElementById('deployments-label-filter');
                ...
                <td>
                    <div class="btn-group" role="group">
                        ...
                        <button class="btn btn-action btn-outline-secondary btn-sm" 
                                onclick="showRollbackModal('${namespace}', '${name}')"
                                title="Rollback">
...
async function addPodToDeployment(namespace, name, currentReplicas) {
    ...
    try {
        await apiFetch(`/api/scale/${namespace}/${name}?replicas=${newReplicas}`, { method: 'POST' });
...
async function applyBulkScale() {
    ...
    selectedDeployments.forEach(key => {
        ...
        if (deployment) {
            ...
            promises.push(
                apiFetch(`/api/scale/${namespace}/${name}?replicas=${newReplicas}`, { method: 'POST' })
                    .then(() => ({ namespace, name, success: true }))
                    .catch(error => ({ namespace, name, success: false, error: error.message }))
            );
        }
    });
...
async function quickScaleSelected() {
    ...
    try {
        const promises = Array.from(selectedDeployments).map(key => {
            const [namespace, name] = key.split('/');
            return apiFetch(`/api/scale/${namespace}/${name}?replicas=${targetReplicas}`, { method: 'POST' });
        });
...
async function scaleAllUp() {
    ...
    try {
        const promises = allDeployments.map(deployment => {
            const current = deployment.replicas || 1;
            return apiFetch(`/api/scale/${deployment.namespace}/${deployment.name}?replicas=${current + 1}`, { method: 'POST' });
        });
...
async function scaleAllDown() {
    ...
    try {
        const promises = allDeployments.map(deployment => {
            ...
            return apiFetch(`/api/scale/${deployment.namespace}/${deployment.name}?replicas=${newReplicas}`, { method: 'POST' });
        });
...
async function showPodLogs(namespace, podName) {
    try {
        ...
        logWindow.document.write(`
            <html>
            <head><title>Logs: ${escapeHtml(podName)}</title>
            <style>
...
async function scaleDeployment() {
    ...
    try {
        const data = await apiFetch(`/api/scale/${namespace}/${name}?replicas=${replicas}`, { method: 'POST' });
...
async function confirmRestart() {
    ...
    try {
        const data = await apiFetch(`/api/restart/${namespace}/${name}`, { method: 'POST' });
...
async function confirmRollback() {
    ...
    const name = currentDeployment.name;
    try {
        const data = await apiFetch(`/api/rollback/${namespace}/${name}`, { method: 'POST' });
        showToast(data.message || 'Rollback initiated', 'success');
...
async function confirmDelete() {
    ...
    try {
        const data = await apiFetch(`/api/deployment/${namespace}/${name}?cascade=${cascade}`, { method: 'DELETE' });
...
async function saveYAML() {
    ...
    try {
        await apiFetch(`/api/deployment/yaml/${namespace}/${name}`, {
            method: 'PUT',
