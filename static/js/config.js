...
function updateSectionHeader(section) {
    const titles = {
        ...
        resourcequotas: { title: 'Resource Quotas', desc: 'View resource quotas per namespace' },
        limitranges: { title: 'Limit Ranges', desc: 'View limit ranges per namespace' },
...
function getSectionIcon(section) {
    const icons = {
        ...
        pvcs: 'fa-database',
        rbac: 'fa-user-shield',
        resourcequotas: 'fa-tachometer-alt',
        limitranges: 'fa-ruler',
...
async function loadCurrentSection() {
    ...
    try {
        ...
        // Обновляем информацию о namespace и кнопку экспорта
        if (currentSection !== 'namespaces' && currentSection !== 'cluster-info') {
            updateNamespaceStats();
        }
        updateExportNamespaceLink();
...
async function loadIngress() {
    ...
    const ns = currentNamespace === 'all' ? 'default' : currentNamespace;
    try {
        const response = await fetch(`/api/ingresses?namespace=${ns}`);
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();
        const list = data.ingresses || [];
        if (list.length === 0) {
            content.innerHTML = `<p class="text-muted text-center py-4">No Ingress resources in namespace <strong>${ns}</strong></p>`;
            return;
        }
        let html = `<div class="table-responsive"><table class="table table-hover"><thead class="table-light"><tr><th>Name</th><th>Hosts</th><th>TLS</th><th>Class</th><th>Age</th><th>Actions</th></tr></thead><tbody>`;
        list.forEach(ing => {
            const hosts = (ing.hosts || []).join(', ') || '—';
            const tls = ing.tls ? 'Yes' : '—';
            html += `<tr>
                <td><strong>${ing.name}</strong></td>
                <td><small>${escapeHtml(hosts)}</small></td>
                <td>${tls}</td>
                <td>${escapeHtml(ing.className || '—')}</td>
                <td>${ing.age}</td>
                <td><button class="btn btn-sm btn-outline-primary" onclick="viewIngressYAML('${ing.namespace}', '${ing.name}')"><i class="fas fa-code"></i> YAML</button></td>
            </tr>`;
        });
        html += '</tbody></table></div>';
        content.innerHTML = html;
    } catch (e) {
        content.innerHTML = `<div class="alert alert-danger"><i class="fas fa-exclamation-circle me-2"></i>${escapeHtml(e.message)}</div>`;
    }
}

async function viewIngressYAML(namespace, name) {
    try {
        const res = await fetch(`/api/ingress/yaml/${namespace}/${name}`);
        if (!res.ok) throw new Error(await res.text());
        const data = await res.json();
        const w = window.open('', '_blank');
        w.document.write(`<html><head><title>Ingress: ${name}</title><style>body{font-family:monospace;padding:16px;background:#f8fafc;} pre{white-space:pre-wrap;}</style></head><body><h3>${name}</h3><pre>${escapeHtml(data.yaml || '')}</pre></body></html>`);
    } catch (e) {
...
async function loadPVCs() {
    ...
    const ns = currentNamespace === 'all' ? 'default' : currentNamespace;
    try {
        const response = await fetch(`/api/pvcs?namespace=${ns}`);
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();
        const list = data.pvcs || [];
        if (list.length === 0) {
            content.innerHTML = `<p class="text-muted text-center py-4">No PVCs in namespace <strong>${ns}</strong></p>`;
            return;
        }
        let html = `<div class="table-responsive"><table class="table table-hover"><thead class="table-light"><tr><th>Name</th><th>Status</th><th>Capacity</th><th>Storage Class</th><th>Volume</th><th>Age</th></tr></thead><tbody>`;
        list.forEach(pvc => {
            html += `<tr>
                <td><strong>${pvc.name}</strong></td>
                <td><span class="badge bg-secondary">${pvc.status}</span></td>
                <td>${escapeHtml(pvc.capacity || '—')}</td>
                <td>${escapeHtml(pvc.storageClass || '—')}</td>
                <td><small>${escapeHtml(pvc.volume || '—')}</small></td>
                <td>${pvc.age}</td>
            </tr>`;
        });
        html += '</tbody></table></div>';
...
async function loadRBAC() {
    const ns = currentNamespace === 'all' ? 'default' : currentNamespace;
    try {
        const [rolesRes, bindingsRes, clusterRolesRes, clusterBindingsRes] = await Promise.all([
            fetch(`/api/roles/${ns}`),
            fetch(`/api/rolebindings/${ns}`),
            fetch('/api/clusterroles'),
            fetch('/api/clusterrolebindings')
        ]);
        const roles = rolesRes.ok ? (await rolesRes.json()).roles || [] : [];
        const bindings = bindingsRes.ok ? (await bindingsRes.json()).roleBindings || [] : [];
        const clusterRoles = clusterRolesRes.ok ? (await clusterRolesRes.json()).roles || [] : [];
        const clusterBindings = clusterBindingsRes.ok ? (await clusterBindingsRes.json()).roleBindings || [] : [];

        document.getElementById('service-accounts').innerHTML = `
            <p class="small text-muted">RoleBindings in <strong>${ns}</strong> (${bindings.length})</p>
            <ul class="list-unstyled small">${bindings.slice(0, 15).map(b => `<li><strong>${b.name}</strong> → ${b.roleRef}<br><small>${(b.subjects || []).join(', ')}</small></li>`).join('') || '<li class="text-muted">None</li>'}
            </ul>`;
        document.getElementById('roles-list').innerHTML = `
            <p class="small text-muted">Roles (${roles.length})</p>
            <ul class="list-unstyled small">${roles.slice(0, 15).map(r => `<li><strong>${r.name}</strong> — ${r.rulesCount} rules, ${r.age}</li>`).join('') || '<li class="text-muted">None</li>'}
            </ul>`;
        document.getElementById('cluster-roles').innerHTML = `
            <p class="small text-muted">ClusterRoles (${clusterRoles.length})</p>
            <ul class="list-unstyled small">${clusterRoles.slice(0, 15).map(r => `<li><strong>${r.name}</strong> — ${r.rulesCount} rules</li>`).join('') || '<li class="text-muted">None</li>'}
            </ul>`;
        await loadRBACVisualization();
    } catch (e) {
        document.getElementById('roles-list').innerHTML = `<div class="text-danger small">${escapeHtml(e.message)}</div>`;
        document.getElementById('cluster-roles').innerHTML = '';
        document.getElementById('service-accounts').innerHTML = '';
        document.getElementById('rbac-viz-content').innerHTML = '<p class="text-muted small">Error loading matrix</p>';
    }
}

async function loadRBACVisualization() {
    const content = document.getElementById('rbac-viz-content');
    if (!content) return;
    const ns = currentNamespace === 'all' ? '' : currentNamespace;
    try {
        const res = await fetch('/api/rbac/visualization?namespace=' + encodeURIComponent(ns || ''));
        const data = await res.json();
        const rows = data.rows || [];
        if (rows.length === 0) {
            content.innerHTML = '<p class="text-muted small">No bindings</p>';
            return;
        }
        let html = '<table class="table table-sm table-hover"><thead><tr><th>Subject</th><th>Role</th><th>Kind</th><th>Namespace</th><th>Permissions</th></tr></thead><tbody>';
        rows.forEach(r => {
            html += '<tr><td>' + escapeHtml(r.subject) + '</td><td>' + escapeHtml(r.role) + '</td><td>' + escapeHtml(r.kind) + '</td><td>' + escapeHtml(r.namespace || '') + '</td><td class="small text-break">' + escapeHtml(r.rulesSummary || '') + '</td></tr>';
        });
        html += '</tbody></table>';
        content.innerHTML = html;
    } catch (e) {
        content.innerHTML = '<p class="text-danger small">' + escapeHtml(e.message) + '</p>';
    }
}

function updateExportNamespaceLink() {
    const btn = document.getElementById('export-namespace-btn');
    if (!btn) return;
    const ns = currentNamespace === 'all' ? 'default' : currentNamespace;
    btn.href = '/api/export/namespace/' + encodeURIComponent(ns);
}

async function loadResourceQuotas() {
    const content = document.getElementById('resourcequotas-content');
    const ns = currentNamespace === 'all' ? '' : currentNamespace;
    try {
        const res = await fetch('/api/resourcequotas?namespace=' + encodeURIComponent(ns || 'default'));
        const data = await res.json();
        const list = data.resourcequotas || [];
        if (list.length === 0) {
            content.innerHTML = '<p class="text-muted text-center py-4">No ResourceQuotas</p>';
            return;
        }
        let html = '<div class="table-responsive"><table class="table table-hover"><thead><tr><th>Name</th><th>Namespace</th><th>Hard</th><th>Used</th><th>Age</th></tr></thead><tbody>';
        list.forEach(rq => {
            const hard = typeof rq.hard === 'object' ? JSON.stringify(rq.hard) : rq.hard;
            const used = typeof rq.used === 'object' ? JSON.stringify(rq.used) : rq.used;
            html += `<tr><td>${escapeHtml(rq.name)}</td><td>${escapeHtml(rq.namespace)}</td><td><small>${escapeHtml(hard)}</small></td><td><small>${escapeHtml(used)}</small></td><td>${escapeHtml(rq.age || '')}</td></tr>`;
        });
        html += '</tbody></table></div>';
        content.innerHTML = html;
    } catch (e) {
...
async function loadLimitRanges() {
    const content = document.getElementById('limitranges-content');
    const ns = currentNamespace === 'all' ? '' : currentNamespace;
    try {
        const res = await fetch('/api/limitranges?namespace=' + encodeURIComponent(ns || 'default'));
        const data = await res.json();
        const list = data.limitranges || [];
        if (list.length === 0) {
            content.innerHTML = '<p class="text-muted text-center py-4">No LimitRanges</p>';
            return;
        }
        let html = '<div class="table-responsive"><table class="table table-hover"><thead><tr><th>Name</th><th>Namespace</th><th>Limits</th><th>Age</th></tr></thead><tbody>';
        list.forEach(lr => {
            const limits = (lr.limits || []).map(l => l.type + ': min=' + JSON.stringify(l.min) + ' max=' + JSON.stringify(l.max)).join('; ');
            html += `<tr><td>${escapeHtml(lr.name)}</td><td>${escapeHtml(lr.namespace)}</td><td><small>${escapeHtml(limits)}</small></td><td>${escapeHtml(lr.age || '')}</td></tr>`;
        });
        html += '</tbody></table></div>';
        content.innerHTML = html;
    } catch (e) {
...
// Просмотр Secret (с загрузкой YAML и декодированных данных)
async function viewSecret(namespace, name) {
    try {
        const [yamlRes, dataRes] = await Promise.all([
            fetch(`/api/secret/yaml/${namespace}/${name}`),
            fetch(`/api/secret/data/${namespace}/${name}`)
        ]);
        if (!yamlRes.ok) throw new Error('Failed to load secret YAML');
        if (!dataRes.ok) throw new Error('Failed to load secret data');
        const yamlData = await yamlRes.json();
        const dataPayload = await dataRes.json();
        const decoded = dataPayload.data || {};
        ...
        document.getElementById('view-secret-type').textContent = dataPayload.type || 'Opaque';
        document.getElementById('view-secret-created').textContent = new Date().toLocaleString();
        document.getElementById('view-secret-yaml').textContent = yamlData.yaml || '';

        window._currentSecretData = decoded;
        const keys = Object.keys(decoded);
        const dataContainer = document.getElementById('view-secret-data');
        if (keys.length === 0) {
            dataContainer.innerHTML = '<p class="text-muted small">No data keys</p>';
        } else {
            dataContainer.innerHTML = keys.map(key => `
                <div class="secret-data-row mb-2 d-flex align-items-center" data-key="${escapeHtml(key)}">
                    <label class="me-2 small text-muted" style="min-width:120px">${escapeHtml(key)}</label>
                    <input type="password" class="form-control form-control-sm flex-grow-1 font-monospace secret-value-input" readonly value="••••••••">
                    <button type="button" class="btn btn-sm btn-outline-secondary ms-2 reveal-secret-btn" onclick="toggleSecretValue(this)" title="Show value">
                        <i class="fas fa-eye"></i>
                    </button>
                </div>
            `).join('');
...
function toggleSecretValue(btn) {
    const row = btn.closest('.secret-data-row');
    const key = row.getAttribute('data-key');
    const value = (window._currentSecretData || {})[key] || '';
...
// Удаление ConfigMap
async function deleteConfigMapConfirm(namespace, name) {
    var ok = await confirmDialog('Удалить ConfigMap', '<p>Удалить ConfigMap <strong>' + escapeHtml(name) + '</strong> в namespace <strong>' + escapeHtml(namespace) + '</strong>?</p>', 'Удалить');
    if (ok) deleteConfigMap(namespace, name);
}
...
// Удаление Secret
async function deleteSecretConfirm(namespace, name) {
    var ok = await confirmDialog('Удалить Secret', '<p>Удалить Secret <strong>' + escapeHtml(name) + '</strong> в namespace <strong>' + escapeHtml(namespace) + '</strong>?</p>', 'Удалить');
    if (ok) deleteSecret(namespace, name);
}
...
// Удаление Namespace
async function deleteNamespaceConfirm(name) {
    if (name === 'default' || name === 'kube-system' || name === 'kube-public') {
        showToast('Cannot delete system namespaces', 'error');
        return;
    }
    var ok = await confirmDialog('Удалить Namespace', '<p>Удалить namespace <strong>' + escapeHtml(name) + '</strong>? Будут удалены все ресурсы в этом namespace.</p>', 'Удалить');
    if (ok) deleteNamespace(name);
}
