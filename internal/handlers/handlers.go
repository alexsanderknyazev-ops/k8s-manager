...
type Handler struct {
	...
	userManager   store.UserManager       // nil если аутентификация не через Postgres
	permManager   store.PermissionManager // nil если нет Postgres
	restConfig    *rest.Config            // для pod exec (WebSocket)
}

func NewHandler(clientset kubernetes.Interface, metricsClient metricsv.Interface, userManager store.UserManager, permManager store.PermissionManager, restConfig *rest.Config) *Handler {
	return &Handler{
		clientset:     clientset,
...
// UserManagementAvailable возвращает true, если управление пользователями доступно (аутентификация через PostgreSQL).
func (h *Handler) UserManagementAvailable() bool {
	return h.userManager != nil
}

func (h *Handler) PermissionManagementAvailable() bool {
	return h.permManager != nil
}
...
// OpenAPIDocsHandler возвращает минимальный OpenAPI 3.0 spec для документации API.
func (h *Handler) OpenAPIDocsHandler(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
		"openapi": "3.0.0",
		"info": gin.H{
		...
		"paths": gin.H{
			"/api/health": gin.H{"get": gin.H{"summary": "Health check"}},
			"/api/pods":   gin.H{"get": gin.H{"summary": "List pods", "parameters": []gin.H{{"name": "namespace", "in": "query"}}}},
			"/api/deployments": gin.H{"get": gin.H{"summary": "List deployments"}},
			"/api/namespaces": gin.H{"get": gin.H{"summary": "List namespaces"}},
			"/api/nodes":      gin.H{"get": gin.H{"summary": "List nodes"}},
		},
	})
}
