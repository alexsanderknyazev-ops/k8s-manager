...
func (h *Handler) GetPodMetricsHandler(c *gin.Context) {
	...
	if h.clientset == nil || h.metricsClient == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{
			"error":          "K8s or Metrics client not ready",
			"clientset":      h.clientset != nil,
			"metrics_client": h.metricsClient != nil,
		})
		return
	}
...
// CheckMetricsHandler - проверка доступности метрик
func (h *Handler) CheckMetricsHandler(c *gin.Context) {
	...
	// Пробуем получить метрики нод
	_, err := h.metricsClient.MetricsV1beta1().NodeMetricses().List(c.Request.Context(), metav1.ListOptions{})

	c.JSON(http.StatusOK, gin.H{
		"metrics_available": err == nil,
		"error": func() string {
			if err != nil {
				return err.Error()
			}
		...
		"tip": "If metrics are not available, install Metrics Server: kubectl apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml",
	})
}

// GetPodMetricsSummaryHandler - краткая сводка по метрикам
func (h *Handler) GetPodMetricsSummaryHandler(c *gin.Context) {
	if h.clientset == nil || h.metricsClient == nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "K8s or Metrics client not ready"})
		return
	}
	...
	// Рассчитываем суммарные метрики
	...
	for _, m := range metrics {
		totalCPU += m.CPURaw
		totalMemory += m.MemoryRaw
		totalCPUPercent += m.CPUPercent
		totalMemoryPercent += m.MemoryPercent
	}

	avgCPUPercent := 0
	avgMemoryPercent := 0
	if totalPods > 0 {
		avgCPUPercent = totalCPUPercent / totalPods
		avgMemoryPercent = totalMemoryPercent / totalPods
	}
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"summary": gin.H{
			...
			"total_cpu_usage":   fmt.Sprintf("%dm", totalCPU),
			"total_memory_usage": k8s.FormatBytes(totalMemory),
			"avg_cpu_percent":   avgCPUPercent,
			"avg_memory_percent": avgMemoryPercent,
		},
		"metrics_count": totalPods,
	})
}