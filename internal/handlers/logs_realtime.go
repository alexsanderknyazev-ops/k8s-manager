...
func (h *Handler) StartLogStreamHandler(c *gin.Context) {
	...
	defer func() {
		...
		logStreamsMu.Unlock()
		stream.stopOnce.Do(func() { close(stream.StopChan) })
		slog.Info("Log stream stopped", "namespace", namespace, "pod", podName)
	}()

	// Отслеживаем закрытие соединения клиентом — тогда сразу останавливаем стрим
	go func() {
		for {
...
func (h *Handler) streamPodLogs(stream *LogStream, clientset kubernetes.Interface) error {
	...
	slog.Debug("Requesting logs for pod", "namespace", stream.Namespace, "pod", stream.Pod, "container", containerName, "follow", stream.Follow, "tail", stream.TailLines)
	...
	if err != nil {
		...
		slog.Warn("log stream failed", "err", err)

		// Попробуем получить логи без указания контейнера
		if containerName != "" {
			slog.Debug("Trying without container name")
