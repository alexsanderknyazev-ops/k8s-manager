...
func (h *Handler) GetPodsHandler(c *gin.Context) {
	...
	if namespace == "all" {
		namespace = "" // corev1.NamespaceAll — список по всем namespace
	}
	limit := int64(500)
	...
	pods, err := h.clientset.CoreV1().Pods(namespace).List(c.Request.Context(), metav1.ListOptions{LabelSelector: labelSelector, Limit: limit})
	if err != nil {
